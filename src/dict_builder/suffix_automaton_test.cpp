...
class SuffixAutomatonTest : public testing::Test {
 protected:
  ...
  void NodeEqualsTest(Node correct_node, Node aut_node, std::string msg) {
    ...
    std::vector<std::pair<char, size_t> > correct_vec(correct_node.edges_begin(), correct_node.edges_end());
    std::vector<std::pair<char, size_t> > aut_vec(aut_node.edges_begin(), aut_node.edges_end());
